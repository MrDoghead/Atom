...
def dequantize_gptq(q, scale, zero, channel_group):
    shape = q.shape
    if channel_group > 1:
        q = q.reshape((int(q.shape[0]/channel_group), -1))
    x = scale * (q - zero)
    return x.reshape(shape)

def quantize_gptq(x, scale, zero, maxq, channel_group, real_quant=False, offset=True):
    if maxq < 0: # TODO
    ...
    if offset:
        ...
        q = torch.clamp(torch.round(x / scale), -(maxq//2+1), maxq//2)
    if not real_quant:
        q = scale * (q - zero)
...
@torch.no_grad()
def quantize_tensor_real(w: torch.tensor, n_bits, group_size, tiling, sym, clip_ratio=1.0, exponential=False) -> torch.tensor:
    savedShape = w.shape
...
@torch.no_grad()
def quantize_activation_wrapper(x: torch.tensor, args, real_quant=False) -> torch.tensor:
    if args.abits >= 16:
        return x 
    ...
    if args.keeper and args.keeper_precision > 0:
        ...
        if args.keeper_precision == 1:
            ...
            if real_quant:
                saved_x, scales_hi, base_hi = quantize_tensor_real(saved_x, n_bits=8, group_size=0, tiling=0, sym=True, exponential=False)
            else:
    ...
    if real_quant:
        return x.view(savedShape), scales_hi, base_hi, scales_lo, base_lo
    else:
        return x.view(savedShape)

@torch.no_grad()
def quantize_attn_v_wrapper(w: torch.tensor, args, real_quant=False) -> torch.tensor:
...
@torch.no_grad()
def quantize_attn_k_wrapper(w: torch.tensor, args, real_quant=False) -> torch.tensor:
    # Input shape: [bsz, self.num_heads, seq_len, self.head_dim]
    # Quantize on head_dim
    assert w.dim() == 4 or  w.dim() == 3
    assert w.shape[-1] == 128
...
class Quantizer(nn.Module):
    ...
    def forward(self, hidden_states, real_quant=False):
        if self.args.static == False or self.scales is None:
            return self.act_quant(hidden_states, real_quant=real_quant)
